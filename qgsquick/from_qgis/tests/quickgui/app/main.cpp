...
int main( int argc, char *argv[] )
{
  ...
  QString qgisPrefixPath = QProcessEnvironment::systemEnvironment().value( "QGIS_PREFIX_PATH" );
  if ( qgisPrefixPath.isEmpty() )
  {
    QgsDebugMsg( QStringLiteral( "FATAL ERROR: QGIS_PREFIX_PATH must be set" ) );
    return EXIT_FAILURE;
  }
  ...
  QString qmlBuildImportPath = QProcessEnvironment::systemEnvironment().value( "QGSQUICK_BUILD_QML_DIR" );
  if ( !qmlBuildImportPath.isEmpty() )
  {
