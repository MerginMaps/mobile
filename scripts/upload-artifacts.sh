...
then
  ...
  python3 ./scripts/uploader.py --source /tmp/${APK_QRCODE} --destination "/$DROPBOX_FOLDER/${APK_QRCODE}" --token DROPBOX_TOKEN > uploader.log 2>&1
  APK_QRCODE_URL=`tail -n 1 uploader.log`
