# Steps to update the qt6 ports 1. Change QT_VERSION below to the new version 2. Set
# QT_UPDATE_VERSION to 1 3. Add any new Qt modules to QT_PORTS 4. Run a build of `qtbase`
# 5. Fix any intermediate failures by adding the module into QT_FROM_GITHUB,
# QT_FROM_GITHUB_BRANCH, or QT_FROM_QT_GIT as appropriate 6. The build should fail with
# "Done downloading version and emitting hashes." This will have changed out the
# vcpkg.json versions of the qt ports and rewritten qt_port_data.cmake 7. Set
# QT_UPDATE_VERSION back to 0
...
if (QT_VERSION VERSION_GREATER_EQUAL 6.6.0)
  list(
    ...
    # New in 6.6.0
    qtgraphs
    # qtvncserver # only commercial qtinsighttracker
  )
endif ()
# qtinsighttracker qtvncserver qtgraphs
...
# 1. These modules are mirrored to github and have branches matching the release
set(QT_FROM_GITHUB_BRANCH qtdeviceutilities)
# 1. These modules are not mirrored to github and not part of the release
set(QT_FROM_QT_GIT "")
# For beta releases uncomment the next two lines and comment the lines with
# QT_FROM_GITHUB, QT_FROM_GITHUB_BRANCH, QT_FROM_QT_GIT set(QT_FROM_QT_GIT ${QT_PORTS})
...
function (
  ...
  elseif ("${qt_port}" IN_LIST QT_FROM_GITHUB_BRANCH)
    ...
    set(mirrors "https://download.qt.io/" "https://mirrors.ocf.berkeley.edu/qt/")
    set(url_subpath
        "${branch_subpath}/qt/${qt_major_minor}/${QT_VERSION}/submodules/${filename}"
    )
...
if (QT_UPDATE_VERSION)
  ...
  foreach (qt_port IN LISTS QT_PORTS)
    ...
    if ("${qt_port}" IN_LIST QT_FROM_QT_GIT)
      ...
      execute_process(
        COMMAND "${GIT}" ls-remote -t "https://code.qt.io/cgit/qt/${qt_port}.git"
                "v${QT_VERSION}" OUTPUT_VARIABLE out
      )
      string(
        SUBSTRING "${out}"
                  0
      ...
      string(APPEND port_data
             "set(${qt_port}_URL \"https://code.qt.io/cgit/qt/${qt_port}.git\")\n"
      )
    else ()
    ...
    file(WRITE "${CMAKE_CURRENT_LIST_DIR}/../../${qt_port}/port.data.cmake"
         "${port_data}"
    )
  endforeach ()
  message("${msg}")
