...
if (VCPKG_TARGET_IS_ANDROID)
  # ANDROID_HOME: canonical SDK environment variable ANDROID_SDK_ROOT: legacy qtbase
  # triplet variable
...
function (qt_download_submodule_impl)
  ...
  if ("${_qarg_SUBMODULE}" IN_LIST QT_FROM_QT_GIT)
    # qtinterfaceframework is not available in the release, so we fall back to a `git
    # clone`.
    vcpkg_from_git(
      OUT_SOURCE_PATH
      SOURCE_PATH
      URL
      "${${_qarg_SUBMODULE}_URL}"
      REF
      "${${_qarg_SUBMODULE}_REF}"
      PATCHES
      ${_qarg_PATCHES}
    )
    if (PORT STREQUAL "qttools") # Keep this for beta & rc's
      ...
      file(COPY "${SOURCE_PATH_QLITEHTML}/"
           DESTINATION "${SOURCE_PATH}/src/assistant/qlitehtml"
      )
      file(COPY "${SOURCE_PATH_LITEHTML}/"
           DESTINATION "${SOURCE_PATH}/src/assistant/qlitehtml/src/3rdparty/litehtml"
      )
    elseif (PORT STREQUAL "qtwebengine")
      vcpkg_from_git(
        OUT_SOURCE_PATH
        SOURCE_PATH_WEBENGINE
        URL
      ...
      if (NOT EXISTS "${SOURCE_PATH}/src/3rdparty/chromium")
        file(RENAME "${SOURCE_PATH_WEBENGINE}/chromium"
             "${SOURCE_PATH}/src/3rdparty/chromium"
        )
      endif ()
    ...
    elseif (NOT DEFINED "${_qarg_SUBMODULE}_HASH")
      message(
        FATAL_ERROR
          "No information for ${_qarg_SUBMODULE} -- add it to QT_PORTS and run qtbase in QT_UPDATE_VERSION mode first"
      )
    else ()
...
function (qt_cmake_configure)
  cmake_parse_arguments(
    ...
    0
    "_qarg"
    "DISABLE_NINJA;DISABLE_PARALLEL_CONFIGURE"
    ""
  ...
  if (NOT
      ...
      "qtwebengine"
      OR QT_IS_LATEST
  ) # qtwebengine requires python2; since 6.3 python3
  ...
  if (NOT
    ...
    list(APPEND _qarg_OPTIONS
         "-DQT_SYNCQT:PATH=${CURRENT_HOST_INSTALLED_DIR}/tools/Qt6/bin/syncqt.pl"
    )
  endif ()
  ...
  string(
    REGEX MATCHALL
          "CMAKE_REQUIRE_FIND_PACKAGE_[^:=]+(:BOOL)?=OFF"
          require_find_package
          "${_qarg_OPTIONS}"
  )
  list(TRANSFORM require_find_package REPLACE "(:BOOL)?=OFF" "")
  list(APPEND _qarg_OPTIONS_MAYBE_UNUSED ${require_find_package})

  # Disable unused warnings for disabled features. Qt might decide to not emit the feature
  # variables if other features are deactivated.
  string(
    REGEX MATCHALL
          "(QT_)?FEATURE_[^:=]+(:BOOL)?=OFF"
          disabled_features
          "${_qarg_OPTIONS}"
  )
  ...
  if (NOT
    ...
    list(APPEND _qarg_OPTIONS
         "-DQT_MKSPECS_DIR:PATH=${CURRENT_HOST_INSTALLED_DIR}/share/Qt6/mkspecs"
    )
  endif ()
  ...
  vcpkg_cmake_configure(
    ...
    -DQT_USE_DEFAULT_CMAKE_OPTIMIZATION_FLAGS:BOOL=ON # We don't want Qt to mess with
                                                      # users toolchain settings.
    -DCMAKE_FIND_PACKAGE_TARGETS_GLOBAL=ON # Because Qt doesn't correctly scope
                                           # find_package calls.
    # -DQT_HOST_PATH=<somepath> # For crosscompiling
    # -DQT_PLATFORM_DEFINITION_DIR=mkspecs/win32-msvc -DQT_QMAKE_TARGET_MKSPEC=win32-msvc
    # -DQT_USE_CCACHE
...
function (qt_fix_prl_files)
  ...
  file(
    GLOB_RECURSE
    prl_files
    "${CURRENT_PACKAGES_DIR}/*.prl"
    "${CURRENT_PACKAGES_DIR}/*.pri"
  )
  foreach (prl_file IN LISTS prl_files)
    ...
    string(
      REPLACE "[QT_INSTALL_PREFIX]/lib/objects-Debug"
              "[QT_INSTALL_LIBS]/objects-Debug"
              _contents
              "${_contents}"
    )
    string(
      REPLACE "[QT_INSTALL_PREFIX]/Qt6/qml"
              "[QT_INSTALL_QML]"
              _contents
              "${_contents}"
    )
    # Note: This only works without an extra if case since QT_INSTALL_PREFIX is the same
...
function (qt_fixup_and_cleanup)
  ...
  set(COMPONENTS)
  file(
    GLOB COMPONENTS_OR_FILES
    LIST_DIRECTORIES true
    "${CURRENT_PACKAGES_DIR}/share/Qt6*"
  )
  ...
  foreach (_glob IN LISTS COMPONENTS_OR_FILES)
    if (IS_DIRECTORY "${_glob}")
      string(
        REPLACE "${CURRENT_PACKAGES_DIR}/share/Qt6"
                ""
                _component
                "${_glob}"
      )
  ...
  foreach (_comp IN LISTS COMPONENTS)
    if (EXISTS "${CURRENT_PACKAGES_DIR}/share/Qt6${_comp}")
      ...
      # Would rather put it into share/cmake as before but the import_prefix correction in
      # vcpkg_cmake_config_fixup is working against that.
    else ()
  ...
  file(GLOB_RECURSE DEBUG_CMAKE_TARGETS
       "${CURRENT_PACKAGES_DIR}/share/**/*Targets-debug.cmake"
  )
  debug_message("DEBUG_CMAKE_TARGETS:${DEBUG_CMAKE_TARGETS}")
  foreach (_debug_target IN LISTS DEBUG_CMAKE_TARGETS)
    vcpkg_replace_string(
      "${_debug_target}"
      "{_IMPORT_PREFIX}/${qt_plugindir}"
      "{_IMPORT_PREFIX}/debug/${qt_plugindir}"
      IGNORE_UNCHANGED
    )
    vcpkg_replace_string(
      "${_debug_target}"
      "{_IMPORT_PREFIX}/${qt_qmldir}"
      "{_IMPORT_PREFIX}/debug/${qt_qmldir}"
      IGNORE_UNCHANGED
    )
  endforeach ()

  file(GLOB_RECURSE STATIC_CMAKE_TARGETS
       "${CURRENT_PACKAGES_DIR}/share/Qt6Qml/QmlPlugins/*.cmake"
  )
  ...
  if (VCPKG_TARGET_IS_WINDOWS AND VCPKG_LIBRARY_LINKAGE STREQUAL "dynamic")
    ...
    file(
      GLOB_RECURSE _installed_dll_files
      RELATIVE "${CURRENT_INSTALLED_DIR}/tools/Qt6/bin"
      "${CURRENT_INSTALLED_DIR}/tools/Qt6/bin/*.dll"
    )
    foreach (_dll_to_remove IN LISTS _installed_dll_files)
      file(GLOB_RECURSE _packaged_dll_file
           "${CURRENT_PACKAGES_DIR}/tools/Qt6/bin/${_dll_to_remove}"
      )
    ...
    endforeach ()
    file(
      GLOB_RECURSE _folders
      LIST_DIRECTORIES true
      "${CURRENT_PACKAGES_DIR}/tools/Qt6/bin/**/"
    )
...
function (qt_install_submodule)
  ...
  if (VCPKG_TARGET_IS_ANDROID)
    ...
    # https://bugreports.qt.io/browse/QTBUG-32618. It requires libc++_shared, cf.
    # <qtbase>/cmake/QtPlatformAndroid.cmake and
  ...
  qt_cmake_configure(
    ...
    ${_qis_CONFIGURE_OPTIONS_MAYBE_UNUSED}
  )
