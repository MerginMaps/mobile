...
if (VCPKG_TARGET_IS_WINDOWS AND NOT VCPKG_TARGET_IS_MINGW)
  ...
  set(CFLAGS
      "/fp:precise /W4 /D_CRT_SECURE_NO_WARNINGS /DYY_NO_UNISTD_H -I./src/headers -I./src/topology -I."
  )
  ...
  # On iOS, spatialite sub-libraries are compiled without SQLITE_CORE, causing every
  # object to reference sqlite3_api as an undefined external. Compile with SQLITE_CORE so
  # the code uses direct SQLite function calls instead of the pointer table.
  ...
  # automake adds the basedir of the generated config to `DEFAULT_INCLUDES`, but
  # libspatialite uses `#include <spatialite/gaiaconfig.h>`.
  file(
    GLOB_RECURSE
    makefiles
  ...
  foreach (makefile IN LISTS makefiles)
    vcpkg_replace_string(
      "${makefile}"
      " -I$(top_builddir)/./src/headers/spatialite"
      " -I$(top_builddir)/./src/headers"
      IGNORE_UNCHANGED
    )
  endforeach ()

  # On iOS libtool has no shared library support. When both lib<name>.la and <name>.la
  # exist in noinst_LTLIBRARIES, both write to the same .libs/lib<name>.a. The -module
  # target runs second and overwrites the archive with empty objects. Fixed by: 1. Adding
  # .NOTPARALLEL: so build is sequential 2. Reversing the order so lib<name>.la is built
  # last
  if (VCPKG_TARGET_IS_IOS)
    file(
      GLOB
    ...
    foreach (makefile IN LISTS ios_makefiles)
      ...
      string(
        REGEX
        REPLACE "noinst_LTLIBRARIES = (lib[^ \t\n]+\\.la) ([^ \t\n]+\\.la)"
  ...
  if (VCPKG_TARGET_IS_MINGW AND VCPKG_LIBRARY_LINKAGE STREQUAL "dynamic")
    if (NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "release")
      ...
      file(RENAME "${CURRENT_PACKAGES_DIR}/lib/mod_spatialite.dll"
           "${CURRENT_PACKAGES_DIR}/plugins/${PORT}/mod_spatialite.dll"
      )
    endif ()
    if (NOT DEFINED VCPKG_BUILD_TYPE OR VCPKG_BUILD_TYPE STREQUAL "debug")
      file(MAKE_DIRECTORY "${CURRENT_PACKAGES_DIR}/debug/plugins/${PORT}")
      file(RENAME "${CURRENT_PACKAGES_DIR}/debug/lib/mod_spatialite.dll"
           "${CURRENT_PACKAGES_DIR}/debug/plugins/${PORT}/mod_spatialite.dll"
      )
    endif ()
  endif ()
endif ()
...
if (NOT ENABLE_GCP AND NOT ENABLE_RTTOPO)
  ...
  file(WRITE "${outfile}" "SpatiaLite[${FEATURES}] is licensed under:\n"
                          "the GPL v2.0 or any subsequent version.\n\n"
  )
endif ()
file(READ "${SOURCE_PATH}/src/control_points/COPYING" gpl)
file(APPEND "${outfile}" "# GPL v2.0 (from src/control_points/COPYING)\n\n" "${gpl}\n")
