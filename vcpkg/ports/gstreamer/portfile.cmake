if (VCPKG_TARGET_IS_WINDOWS)
  set(PATCHES plugin-base-disable-no-unused.patch)
endif ()
...
vcpkg_from_gitlab(
  ...
  "${VERSION}"
  SHA512
  0d69896d0a83452320df0d0f56c710df1365a259cd3f48dc7cd4df18d45b27caea7174aafa15ae5eb8637ccdef192c1047185b369b7232db4eaacbc57ffaaa22
  HEAD_REF
  main
  PATCHES
  fix-clang-cl.patch
...
if (VCPKG_TARGET_IS_OSX)
  # In Darwin platform, there can be an old version of `bison`, Which can't be used for
  # `gst-build`. It requires 2.4+
  ...
  if (NOT (BISON_MAJOR GREATER_EQUAL 2 AND BISON_MINOR GREATER_EQUAL 4))
    message(
      WARNING
        "'bison' upgrade is required. Please check the https://stackoverflow.com/a/35161881"
    )
  endif ()
endif ()
...
vcpkg_check_features(
  ...
  base
  gl-graphene
  gst-plugins-base:gl-graphene
  alsa
  gst-plugins-base:alsa
  ogg
  gst-plugins-base:ogg
  opus-base
  gst-plugins-base:opus
  pango
  gst-plugins-base:pango
  vorbis
  gst-plugins-base:vorbis
  x11-base
  gst-plugins-base:x11
  x11-base
  gst-plugins-base:xshm
  plugins-good
  good
  bzip2-good
  gst-plugins-good:bz2
  cairo
  ...
  mpg123
  gst-plugins-good:mpg123
  png
  gst-plugins-good:png
  speex
  gst-plugins-good:speex
  taglib
  gst-plugins-good:taglib
  vpx
  gst-plugins-good:vpx
  plugins-ugly
  ugly
  x264
  ...
  gst-plugins-bad:aom
  assrender
  gst-plugins-bad:assrender
  bzip2-bad
  gst-plugins-bad:bz2
  chromaprint
  gst-plugins-bad:chromaprint
  closedcaption
  gst-plugins-bad:closedcaption
  colormanagement
  gst-plugins-bad:colormanagement
  dash
  gst-plugins-bad:dash
  dc1394
  gst-plugins-bad:dc1394
  dtls
  gst-plugins-bad:dtls
  faad
  gst-plugins-bad:faad
  fdkaac
  gst-plugins-bad:fdkaac
  fluidsynth
  gst-plugins-bad:fluidsynth
  libde265
  gst-plugins-bad:libde265
  microdns
  gst-plugins-bad:microdns
  modplug
  gst-plugins-bad:modplug
  nvcodec
  gst-plugins-bad:nvcodec
  openal
  gst-plugins-bad:openal
  openh264
  gst-plugins-bad:openh264
  openjpeg
  gst-plugins-bad:openjpeg
  openmpt
  gst-plugins-bad:openmpt
  opus-bad
  gst-plugins-bad:opus
  smoothstreaming
  gst-plugins-bad:smoothstreaming
  sndfile
  ...
  gst-plugins-bad:srt
  srtp
  gst-plugins-bad:srtp
  webp
  gst-plugins-bad:webp
  webrtc
  gst-plugins-bad:webrtc
  wildmidi
  gst-plugins-bad:wildmidi
  x11-bad
  gst-plugins-bad:x11
  x265
  gst-plugins-bad:x265
  asio
  gst-plugins-bad:asio
)
...
#
# Rationale for added options Common options are added below systematically Feature
# options are added below only if the feature needs an external dependency Feature options
# that are dependent on the operating system type (like wasapi or osxaudio) are set to
# auto Every other feature options are made available if the dependency is available on
# vcpkg and if the plugin has managed to build during tests
#
...
vcpkg_configure_meson(
  ...
  -Dorc=disabled # gstreamer requires a specific version of orc which is not available in
                 # vcpkg
  -Ddoc=disabled
  -Dgtk_doc=disabled
  # gstreamer
  ...
  -Dgst-plugins-bad:gs=disabled # Error during plugin configuration (abseil pkg-config
                                # file missing)
  ...
  -Dgst-plugins-bad:lv2=disabled # Error during plugin configuration (lilv pkg-config file
                                 # missing)
...
list(APPEND GST_LIBEXEC_TOOLS gst-plugin-scanner)
...
if ("plugins-bad" IN_LIST FEATURES)
  list(APPEND GST_BIN_TOOLS gst-transcoder-1.0)
endif ()
...
vcpkg_copy_tools(
  ...
  ${GST_LIBEXEC_TOOLS}
  SEARCH_DIR
  "${CURRENT_PACKAGES_DIR}/libexec/gstreamer-1.0"
  AUTO_CLEAN
)
...
if (VCPKG_LIBRARY_LINKAGE STREQUAL "static")
  ...
  vcpkg_replace_string(
    "${CURRENT_PACKAGES_DIR}/include/gstreamer-1.0/gst/gstconfig.h"
    "!defined(GST_STATIC_COMPILATION)" "0"
  )
endif ()
...
if (VCPKG_LIBRARY_LINKAGE STREQUAL "dynamic")
  ...
  if (NOT VCPKG_BUILD_TYPE)
    file(
      GLOB
      DBG_BINS
      "${CURRENT_PACKAGES_DIR}/debug/lib/gstreamer-1.0/${CMAKE_SHARED_LIBRARY_PREFIX}*${CMAKE_SHARED_LIBRARY_SUFFIX}"
  ...
  file(
    GLOB
    REL_BINS
    "${CURRENT_PACKAGES_DIR}/lib/gstreamer-1.0/${CMAKE_SHARED_LIBRARY_PREFIX}*${CMAKE_SHARED_LIBRARY_SUFFIX}"
  ...
  if (EXISTS "${_file}")
    ...
    string(
      REPLACE [[toolsdir=${exec_prefix}/bin]]
              "toolsdir=\${prefix}/../tools/${PORT}"
              _contents
              "${_contents}"
    )
    string(
      REPLACE [[pluginscannerdir=${libexecdir}/gstreamer-1.0]]
              "pluginscannerdir=\${prefix}/../tools/${PORT}"
              _contents
              "${_contents}"
    )
    string(
      REPLACE [[pluginsdir=${libdir}/gstreamer-1.0]]
              "pluginsdir=\${prefix}/plugins/${PORT}"
              _contents
              "${_contents}"
    )
