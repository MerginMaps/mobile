...
MMPrivateComponents.MMBaseSingleLineInput {
  ...
  property int type: MMFilterBaseInput.InputType.Text

  // date and dropdown types open pickers instead of accepting keyboard input
  textField.readOnly: root.type === MMFilterBaseInput.InputType.Date || root.type === MMFilterBaseInput.InputType.Dropdown
  textField.inputMethodHints: root.type === MMFilterBaseInput.InputType.Number ? Qt.ImhFormattedNumbersOnly : Qt.ImhNone
  textField.color: {
  ...
  rightContent: MMComponents.MMIcon {
    ...
    source: {
      ...
      if ( root.type === MMFilterBaseInput.InputType.Date ) return __style.calendarIcon
      if ( root.type === MMFilterBaseInput.InputType.Dropdown ) return __style.arrowDownIcon
  ...
  rightContentVisible: {
    if ( root.type === MMFilterBaseInput.InputType.Text || root.type === MMFilterBaseInput.InputType.Number ) return root.checked
    return true
  }
  ...
  onTextChanged: {
    if ( root.type !== MMFilterBaseInput.InputType.Dropdown ) {
  ...
  onRightContentClicked: {
    if ( ( root.type === MMFilterBaseInput.InputType.Text || root.type === MMFilterBaseInput.InputType.Number ) && root.checked ) {
      textField.clear()
      root.checked = false
    }
  }
