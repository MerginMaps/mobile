...
Item {
  ...
  function constructHighlights() {
    if (!featureLayerPair || !mapSettings)
      return;
    ...
    if (data.length > dataStartIndex) {
      ...
      } else // line or polygon
      {
        ...
        while (i < data.length) {
          ...
          elements.push(componentMoveTo.createObject(objOwner, {
                "x": data[i],
                "y": data[i + 1]
              }));
          // Draw lines for rest of points in the segment
          for (k = i + 2; k < i + pointsCount * 2; k += 2) {
            elements.push(componentLineTo.createObject(objOwner, {
                  "x": data[k],
                  "y": data[k + 1]
                }));
          }
        ...
        if (recordingInProgress && guideLineAllowed) {
          // construct a guide line / polygon
          if (geometryType === 2 && elements.length > 2) {
            ...
            newGuideLineElements.push(componentLineTo.createObject(guideLine, {
                  "x": centerPoint.x,
                  "y": centerPoint.y
                }));
            ...
            newGuideLineElements.push(componentMoveTo.createObject(guideLine, {
                  "x": elements[elements.length - 1].x,
                  "y": elements[elements.length - 1].y
                }));
            let centerPoint = crosshairPoint();
            newGuideLineElements.push(componentLineTo.createObject(guideLine, {
                  "x": centerPoint.x,
  ...
  function updateGuideLine() {
    ...
    if (hasPolygon && elements.length > 2) {
      ...
      let centerPoint = crosshairPoint();
      elements.push(componentLineTo.createObject(guideLine, {
            "x": centerPoint.x,
            "y": centerPoint.y
          }));
