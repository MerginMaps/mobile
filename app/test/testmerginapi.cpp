...
TestMerginApi::TestMerginApi( MerginApi *api )
{
  ...
  mLocalProjectsModel = std::make_unique<ProjectsModel>();
  ...
  mWorkspaceProjectsModel = std::make_unique<ProjectsModel>();
...
void TestMerginApi::initTestCase()
{
  ...
  const QDir testDataDir( TEST_DATA_DIR );
  ...
  if ( !QDir( testDataDir.path() + "/.." ).mkpath( "temp_extra_projects" ) )
  {
    qDebug() << QString( "Failed to create directory for temp_extra_projects" );
  };
  const QString projectsExtraDir = testProjectsExtraDir.canonicalPath() + "/";
...
{
  const QString projectName = "testListProject";
  ...
  QVERIFY( !TestUtils::findProjectByName( projectFullName, projects ).isValid() );
  QVERIFY( !TestUtils::findProjectByName( projectFullName, mApi->localProjectsManager().projects().values() ).isValid() );
...
{
  const QString projectName = "testListProjectByName";
  ...
  const QByteArray oldToken = mApi->userAuth()->authToken();
  ...
  const QDateTime now = QDateTime::currentDateTimeUtc().addSecs( -3 );
  mApi->userAuth()->setTokenExpiration( now );

  const QStringList projects{ CoreUtils::getFullProjectName( mWorkspaceName, projectName ) };
  ...
  const MerginProjectsList receivedProjects = projectListFromSpy( responseReceived );
  QVERIFY( receivedProjects.count() == 1 );

  const MerginProject &ourProject = receivedProjects.at( 0 );
...
{
  // create the project on the server (the content is not important)
  const QString projectName = "testDownloadProject";
  const QString projectNamespace = mWorkspaceName;
  const QString projectFullName = CoreUtils::getFullProjectName( projectNamespace, projectName );
  ...
  const QString projectId = projectIdFromProjectFullName( mApi, projectNamespace, projectName );

  // try to download the project
  QSignalSpy spy( mApi, &MerginApi::syncProjectFinished );
  mApi->pullProject( projectFullName, projectId );
  ...
  QVERIFY( mApi->localProjectsManager().projectFromProjectId( projectId ).isValid() );

  // update model to have the latest info
  refreshProjectsModel( ProjectsModel::LocalProjectsModel );

  Project project = mLocalProjectsModel->projectFromId( projectId );
  ...
  bool downloadSuccessful = mApi->localProjectsManager().projectFromProjectId( projectId ).isValid();
...
void TestMerginApi::testDownloadProjectSpecChars()
{
  ...
  const QString projectName = "testDownloadProjectSpecChars";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  const QString projectDir = mApi->projectsPath() + "/" + projectName + "/";

  // TODO: maybe use createRemoteProject instead of this
  ...
  mApi->createProject( mWorkspaceName, projectName, CoreUtils::uuidWithoutBraces( QUuid::createUuid() ), true );
  ...
  const QString projectId = projectIdFromProjectFullName( mApi, mWorkspaceName, projectName );
  mApi->localProjectsManager().addMerginProject( projectDir, mWorkspaceName, projectName, projectId );

  // Copy data
  const QString sourcePath = mTestDataPath + "/" + TEST_PROJECT_NAME + "/";
  ...
  const QString specChars( "_-. " ); // can't start with dot or space
  const QString newProjectFileName = QString( "%1.qgs" ).arg( specChars );
  QVERIFY( projectFile.rename( projectDir + "/" + newProjectFileName ) );

  // Upload data
  QSignalSpy spy2( mApi, &MerginApi::syncProjectFinished );
  mApi->pushProject( projectFullName, projectId );
  ...
  downloadRemoteProject( mApiExtra, projectFullName, projectId );
...
{
  const QString projectName = "testCancelDownloadProject";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  // Test download and cancel after transaction starts
...
{
  const QString projectName = "testCreateProjectTwice";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
...
void TestMerginApi::testDeleteNonExistingProject()
{
  ...
  mApi->deleteProject( CoreUtils::uuidWithoutBraces( QUuid() ) );
  QVERIFY( spy.wait( TestUtils::SHORT_REPLY ) );
...
{
  // Create a project
  const QString projectName = "testCreateDeleteProject";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  QVERIFY( mWorkspaceProjectsModel->rowCount() );
  Q_ASSERT( TestUtils::findProjectByName( projectFullName, projects ).isValid() );
...
{
  const QString projectName = "testUploadProject";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  mApiExtra->createProject( mWorkspaceName, projectName, CoreUtils::uuidWithoutBraces( QUuid::createUuid() ), true );
  ...
  const QString projectId = TestUtils::findProjectByName( projectFullName, projects ).id();
  mApi->localProjectsManager().addMerginProject( projectDir, mWorkspaceName, projectName, projectId );

  // project info does not have any version information yet
  Project project0 = mLocalProjectsModel->projectFromId( projectId );
  ...
  Project project1 = mLocalProjectsModel->projectFromId( projectId );
  ...
  Project project2 = mLocalProjectsModel->projectFromId( projectId );
  QVERIFY( project2.isLocal() && !project2.isMergin() );
  QCOMPARE( project2.local.localVersion, -1 );

  refreshProjectsModel( ProjectsModel::LocalProjectsModel );

  project2 = mLocalProjectsModel->projectFromId( projectId );
  QVERIFY( project2.isLocal() && project2.isMergin() );
  QCOMPARE( project2.local.localVersion, -1 );
  ...
  Project project3 = mLocalProjectsModel->projectFromId( projectId );
...
void TestMerginApi::testMultiChunkUploadDownload()
{
  ...
  const QString bigFilePath = mApi->projectsPath() + "/" + projectName + "/" + "big_file.dat";
  ...
  const QByteArray checksum = CoreUtils::calculateChecksum( bigFilePath );
  QVERIFY( !checksum.isEmpty() );

  // upload
  uploadRemoteProject( mApi, projectFullName, projectId );

  // download again
  deleteLocalProject( mApi, projectId );
  QVERIFY( !QFileInfo::exists( bigFilePath ) );
  downloadRemoteProject( mApi, projectFullName, projectId );

  // verify it's there and with correct content
  const QByteArray checksum2 = CoreUtils::calculateChecksum( bigFilePath );
  QVERIFY( QFileInfo::exists( bigFilePath ) );
...
void TestMerginApi::testEmptyFileUploadDownload()
{
  ...
  const QString emptyFileDestinationPath = mApi->projectsPath() + "/" + projectName + "/" + TEST_EMPTY_FILE_NAME;
  ...
  const QByteArray checksum = CoreUtils::calculateChecksum( emptyFileDestinationPath );
  QVERIFY( !checksum.isEmpty() );
  ...
  // verify it's there and with correct content
  const QByteArray checksum2 = CoreUtils::calculateChecksum( emptyFileDestinationPath );
  QVERIFY( QFileInfo::exists( emptyFileDestinationPath ) );
  QCOMPARE( checksum, checksum2 );
}
...
{
  const QString projectName = "testPushAddedFile";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  Project project0 = mWorkspaceProjectsModel->projectFromId( projectId );
  QVERIFY( project0.isLocal() && project0.isMergin() );
  QCOMPARE( project0.local.localVersion, 1 );
  ...
  // add a single file
  const QString newFilePath = mApi->projectsPath() + "/" + projectName + "/added.txt";
  ...
  Project project1 = mWorkspaceProjectsModel->projectFromId( projectId );
  QVERIFY( project1.isLocal() && project1.isMergin() );
  QCOMPARE( project1.local.localVersion, 1 );
  ...
  Project project2 = mWorkspaceProjectsModel->projectFromId( projectId );
  QVERIFY( project2.isLocal() && project2.isMergin() );
  ...
  Project project3 = mWorkspaceProjectsModel->projectFromId( projectId );
  QVERIFY( project3.isLocal() && project3.isMergin() );
...
{
  const QString projectName = "testPushModifiedFile";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  const QString filename = mApi->projectsPath() + "/" + projectName + "/project.qgs";
  QFile file( filename );
  QVERIFY( file.open( QIODevice::WriteOnly ) );
...
{
  const QString projectName = "testPushNoChanges";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
...
{
  const QString projectName = "testUpdateAddedFile";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString extraProjectDir = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  uploadRemoteProject( mApiExtra, projectFullName, projectId );
...
{
  const QString projectName = "testUpdateRemovedFiles";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString extraProjectDir = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
...
{
  const QString projectName = "testUpdateRemovedVsModifiedFiles";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString extraProjectDir = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
...
{
  const QString projectName = "testConflictRemoteUpdateLocalUpdate";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString extraProjectDir = mApiExtra->projectsPath() + "/" + projectName;
  const QString filename = projectDir + "/test1.txt";
  const QString extraFilename = extraProjectDir + "/test1.txt";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  const QString conflictFilename = projectDir + "/test1 (conflicted copy, " + mUsername + " v1).txt";
  ...
  const QString conflictFilename2 = projectDir + "/test1 (conflicted copy, " + mUsername + " v3).txt";
...
{
  const QString projectName = "testConflictRemoteAddLocalAdd";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString extraProjectDir = mApiExtra->projectsPath() + "/" + projectName;
  const QString filename = projectDir + "/test-new-file.txt";
  const QString extraFilename = extraProjectDir + "/test-new-file.txt";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  const QString conflictFilename = projectDir + "/test-new-file (conflicted copy, " + mUsername + " v1).txt";
...
{
  const QString projectName = "testEditConflictScenario";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString extraProjectDir = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );

  // folder rebase_edit_conflict
  const QString dataProjectDir = TestUtils::testDataDir() + "/" + QStringLiteral( "rebase_edit_conflict" );
  qDebug() << "About to create the project";
  createRemoteProject( mApiExtra, mWorkspaceName, projectName, mTestDataPath + "/" + TEST_PROJECT_NAME + "/" );
  qDebug() << "Project has been created!";
  const QString dbName = QStringLiteral( "data.gpkg" );
  const QString baseDB = dataProjectDir + QStringLiteral( "/base.gpkg" );
  const QString localChangeDB = dataProjectDir + QStringLiteral( "/local-change.gpkg" );
  const QString remoteChangeDB = dataProjectDir + QStringLiteral( "/remote-change.gpkg" );
  const QString projectId = projectIdFromProjectFullName( mApi, mWorkspaceName, projectName );
...
{
  const QString projectName = "testUploadWithUpdate";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString extraProjectDir = mApiExtra->projectsPath() + "/" + projectName;
  const QString filenameLocal = projectDir + "/test-new-local-file.txt";
  const QString filenameRemote = projectDir + "/test-new-remote-file.txt";
  const QString extraFilenameRemote = extraProjectDir + "/test-new-remote-file.txt";
...
{
  const QString projectName = "testDiffUpload";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  const ProjectDiff diff = MerginApi::localProjectChanges( projectDir );
  ...
  const QString changes = GeodiffUtils::diffableFilePendingChanges( projectDir, "base.gpkg", true );
  const GeodiffUtils::ChangesetSummary summary = GeodiffUtils::parseChangesetSummary( changes );
...
{
  const QString projectName = "testDiffSubdirsUpload";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  GeodiffUtils::ChangesetSummary expectedSummary;
  expectedSummary["simple"] = GeodiffUtils::TableSummary( 0, 1, 0 );
  const QString changes = GeodiffUtils::diffableFilePendingChanges( projectDir, base, true );
...
{
  const QString projectName = "testDiffUpdateBasic";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString projectDirExtra = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  const QgsVectorLayer *vl0 = new QgsVectorLayer( projectDir + "/base.gpkg|layername=simple", "base", "ogr" );
  QVERIFY( vl0->isValid() );
  ...
  const bool r1 = QFile::copy( mTestDataPath + "/added_row.gpkg", projectDirExtra + "/base.gpkg" );
  QVERIFY( r0 && r1 );
  uploadRemoteProject( mApiExtra, projectFullName, projectId );
  ...
  const QgsVectorLayer *vl = new QgsVectorLayer( projectDir + "/base.gpkg|layername=simple", "base", "ogr" );
  QVERIFY( vl->isValid() );
...
{
  const QString projectName = "testDiffUpdateWithRebase";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString projectDirExtra = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
...
{
  const QString projectName = "testDiffUpdateWithRebaseFailed";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString projectDirExtra = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
...
{
  const QString projectName = "testUpdateWithDiffs";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString projectDirExtra = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  const bool r2 = QFile::remove( projectDirExtra + "/base.gpkg" );
  const bool r3 = QFile::copy( mTestDataPath + "/added_row_2.gpkg", projectDirExtra + "/base.gpkg" );
  QVERIFY( r2 && r3 );
...
{
  const QString projectName = "testUpdateWithMissedVersion";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString projectDirExtra = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  // step 1
...
{
  const QString projectName = "testMigrateProject";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  const QStringList entryList = QDir( projectDir ).entryList( QDir::NoDotAndDotDot | QDir::Dirs );
  ...
  // project ID changed after the push to server
  localProjectId = TestUtils::findProjectByName( projectFullName, mApi->mLocalProjects.projects().values() ).id();
  // remove local copy of project
  deleteLocalProject( mApi, localProjectId );
  QVERIFY( !QFileInfo::exists( projectDir ) );

  // download the project
  downloadRemoteProject( mApi, projectFullName, localProjectId );

  // verify that all files have been uploaded
  const QStringList entryList2 = QDir( projectDir ).entryList( QDir::NoDotAndDotDot | QDir::Dirs );
  QCOMPARE( entryList, entryList2 );
}

void TestMerginApi::testMigrateProjectAndSync()
{
  const QString projectName = "testMigrateProjectAndSync";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString projectDirExtra = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  // step 3
  downloadRemoteProject( mApiExtra, projectFullName, localProjectId );
  bool r0 = QFile::remove( projectDirExtra + "/base.gpkg" );
  bool r1 = QFile::copy( mTestDataPath + "/added_row.gpkg", projectDirExtra + "/base.gpkg" );
  QVERIFY( r0 && r1 );
  uploadRemoteProject( mApiExtra, projectFullName, localProjectId );
  ...
  QVERIFY( r0 && r1 );
  uploadRemoteProject( mApi, projectFullName, localProjectId );
...
{
  const QString projectName = "testMigrateDetachProject";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  // detach project
  mApi->detachProjectFromMergin( localProjectId );
...
{
  // Create a project
  const QString projectName = "testSelectiveSync";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString projectDirExtra = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  // Create photo files
  const QDir dir;
  const QString photoPath( projectDir + "/subdir" );
  if ( !dir.exists( photoPath ) )
    if ( !dir.mkpath( photoPath ) )
    {
      qDebug() << QString( "Failed to create directory for %1" ).arg( photoPath );
    };
  ...
  downloadRemoteProject( mApiExtra, projectFullName, projectId );
  const QString configFilePathExtra( projectDirExtra + "/mergin-config.json" );
  ...
  const QFile fileExtra( projectDirExtra + "/photo.jpg" );
  QVERIFY( !fileExtra.exists() );

  const QFile fileExtra1( projectDirExtra + "/subdir/photo.jpg" );
  ...
  const QFile file2( projectDir + "/" + "photoExtra.png" );
...
{
  // Create a project
  const QString projectName = "testSelectiveSyncSubfolder";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString projectDirExtra = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  // Create photo files
  const QDir dir;
  const QString photoPath( projectDir + "/photos" );
  if ( !dir.exists( photoPath ) )
    if ( !dir.mkpath( photoPath ) )
    {
      qDebug() << QString( "Failed to create directory %1" ).arg( photoPath );
    };
  ...
  const QString photoPathExtra( projectDirExtra + "/photos" );

  const QFile fileExtra( photoPathExtra + "/" + "photoA.jpg" );
  QVERIFY( !fileExtra.exists() );

  const QFile fileExtra1( photoPathExtra + "/" + "photoB.png" );
  QVERIFY( !fileExtra1.exists() );
  ...
  const QDir photoDirExtra( photoPathExtra );
  if ( !photoDirExtra.exists() ) // if the subfolder contained only photos, it was not even created in Client 2
    ...
    {
      qDebug() << QString( "Failed to create directory for %1" ).arg( photoPathExtra );
    };
  ...
  const QFile fileExtra2( photoPath + "/" + "photoC.jpg" );
  QVERIFY( !fileExtra2.exists() );

  const QFile fileExtra3( projectDir + "/" + "photoD.png" );
  QVERIFY( fileExtra3.exists() );

  // Check that photos were not deleted for Client 1
  const QFile file2( photoPath + "/" + "photoA.jpg" );
  QVERIFY( file2.exists() );

  const QFile file3( photoPath + "/" + "photoB.png" );
...
{
  // Create a project
  const QString projectName = "testSelectiveSyncAddConfigToExistingProject";
  const QString projectDir = mApi->projectsPath() + "/" + projectName;
  const QString projectDirExtra = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  const QFile fileExtra2( photoPathExtra + "/" + "photoC.png" );
  QVERIFY( !fileExtra2.exists() );
}

void TestMerginApi::testSelectiveSyncRemoveConfig()
{
  const QString serverMirrorDataPath = mApi->projectsPath() + "/" + "serverMirror";
  const QDir serverMirrorDataDir( serverMirrorDataPath );
  if ( !serverMirrorDataDir.exists() )
    if ( !serverMirrorDataDir.mkpath( serverMirrorDataPath ) )
    {
      qDebug() << QString( "Failed to create directory for %1" ).arg( serverMirrorDataPath );
    };
  ...
  const QString projectName = "testSelectiveSyncRemoveConfig";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );

  const QString projectClient1 = mApi->projectsPath() + "/" + projectName;
  const QString projectClient2 = mApiExtra->projectsPath() + "/" + projectName;
  const QString projectServer = serverMirror->projectsPath() + "/" + projectName;
  ...
  // Create photo files
  const QDir dir;
  const QString photoPathClient1( projectClient1 + "/photos" );
  if ( !dir.exists( photoPathClient1 ) )
    if ( !dir.mkpath( photoPathClient1 ) )
    {
      qDebug() << QString( "Failed to create directory for %1" ).arg( photoPathClient1 );
    };
  ...
  uploadRemoteProject( mApi, projectFullName, projectId );
  downloadRemoteProject( serverMirror, projectFullName, projectId );
  ...
  uploadRemoteProject( serverMirror, projectFullName, projectId );
  downloadRemoteProject( mApiExtra, projectFullName, projectId );
  ...
  if ( !photoDirExtra.exists() )
    if ( !photoDirExtra.mkpath( photoPathClient2 ) )
    {
      qDebug() << QString( "Failed to create directory for %1" ).arg( photoPathClient2 );
    };
  ...
  // Sync removed config
  uploadRemoteProject( serverMirror, projectFullName, projectId );
  downloadRemoteProject( mApi, projectFullName, projectId ); // download back to apply the changes -> should download photos
  ...
  const QString photoPathServer = serverMirrorDataPath + "/" + projectName + "/" + "photos";

  // check that all clients have all photos
  const QStringList photos( { "photoA.jpg", "photoB.png", "photoC.png", "photoC2-extra.png" } );
...
void TestMerginApi::testSelectiveSyncDisabledInConfig()
{
  ...
  // Create a project with photos and mergin-config
  const QString projectName = "testSelectiveSyncDisabledInConfig";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  const QStringList photos( { "photoA.jpg", "photoB.png", "photoC1-5.png", "photoC2-3.png", "photoC2-extra.png" } );
  for ( const QString &photo : photos )
  {
    QFile photo1( photoPathClient1 + "/" + photo );
    QFile photo2( photoPathClient2 + "/" + photo );
...
void TestMerginApi::testSelectiveSyncChangeSyncFolder()
{
  ...
  // Create a project with photos and mergin-config
  const QString projectName = "testSelectiveSyncChangeSyncFolder";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  const QString &photoPathClient2( projectClient2 );
...
void TestMerginApi::testSelectiveSyncCorruptedFormat()
{
  ...
  // Create a project with photos and mergin-config
  const QString projectName = "testSelectiveSyncCorruptedFormat";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  const QString photoPathClient2( projectClient2 + "/" + "photos" );

  const QFile fileExtra( photoPathClient2 + "/" + "photoC1-A.jpg" );
  QVERIFY( fileExtra.exists() );

  const QFile fileExtra1( photoPathClient2 + "/" + "photoC1-B.png" );
  QVERIFY( fileExtra1.exists() );
}
...
void TestMerginApi::testSynchronizationViaManager()
{
  ...
  const QString projectName( QStringLiteral( "testSynchronizationViaManager" ) );
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  QVERIFY( syncManager.hasPendingSync( projectId ) );
...
{
  const QString projectName = QStringLiteral( "testAutosync" );
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  const QString projectDir = QDir::tempPath() + "/" + projectName;
  const QString projectFilename = "quickapp_project.qgs";
...
void TestMerginApi::testRegisterAndDelete()
{
  ...
  const QString password = mApi->userAuth()->password();

  const QString quiteRandom = CoreUtils::uuidWithoutBraces( QUuid::createUuid() ).right( 15 ).replace( "-", "" );
  const QString email = "test_" + quiteRandom + "@nonexistent.email.com";
...
void TestMerginApi::testCreateWorkspace()
{
  ...
  const QString username = TestUtils::generateUsername();
  const QString password = TestUtils::generatePassword();
  const QString email = TestUtils::generateEmail();
...
void TestMerginApi::testExcludeFromSync()
{
  ...
  const QString selectiveSyncDir( mApi->projectsPath() + "/testExcludeFromSync" );
...
MerginProjectsList TestMerginApi::getProjectList( const QString &tag )
{
...
MerginProjectsList TestMerginApi::projectListFromSpy( QSignalSpy &spy )
{
  ...
  if ( !spy.isEmpty() )
  {
    const QList<QVariant> response = spy.takeFirst();

    // get projects emitted from MerginAPI, it is first argument in listProjectsFinished signal
    if ( !response.isEmpty() )
...
int TestMerginApi::serverVersionFromSpy( QSignalSpy &spy )
{
  ...
  if ( !spy.isEmpty() )
  {
    ...
    // get version number emitted from MerginApi::syncProjectFinished, it is third argument
...
void TestMerginApi::createRemoteProject( MerginApi *api, const QString &projectNamespace, const QString &projectName, const QString &sourcePath, const bool force )
{
  if ( force )
  {
  ...
  api->createProject( projectNamespace, projectName, CoreUtils::uuidWithoutBraces( QUuid::createUuid() ), true );
  QVERIFY( spy.wait( TestUtils::SHORT_REPLY ) );
  QCOMPARE( spy.count(), 1 );
  QCOMPARE( spy.takeFirst().at( 1 ).toBool(), true );

  // Copy data
  const QString projectDir = api->projectsPath() + "/" + projectName + "/";
  InputUtils::cpDir( sourcePath, projectDir );

  // make MerginApi aware of the project and its directory
  const QString merginProjectId = projectIdFromProjectFullName( api, projectNamespace, projectName );
  api->localProjectsManager().addMerginProject( projectDir, projectNamespace, projectName, merginProjectId );

  // Upload data
  QSignalSpy spy3( api, &MerginApi::syncProjectFinished );
  api->pushProject( CoreUtils::getFullProjectName( projectNamespace, projectName ), merginProjectId );
  QVERIFY( spy3.wait( TestUtils::LONG_REPLY ) );
  QCOMPARE( spy3.count(), 1 );
  const QList<QVariant> arguments = spy3.takeFirst();
  const int version = arguments.at( 2 ).toInt();
  QCOMPARE( version, 1 );
  ...
  api->localProjectsManager().removeLocalProject( merginProjectId );
...
QString TestMerginApi::projectIdFromProjectFullName( MerginApi *api, const QString &projectNamespace, const QString &projectName )
{
  ...
  QNetworkReply *r = api->getProjectInfo( projectFullName, QString() );
  Q_ASSERT( r );
...
void TestMerginApi::deleteRemoteProjectNow( MerginApi *api, const QString &projectNamespace, const QString &projectName )
{
  ...
  const QString projectId = projectIdFromProjectFullName( api, projectNamespace, projectName );
  if ( projectId.isEmpty() )
  {
    // probably no such project exists on server
    return;
  }

  QNetworkRequest request = api->getDefaultRequest();
  const QUrl url( api->mApiRoot + QStringLiteral( "/v2/projects/%1" ).arg( projectId ) );
  request.setUrl( url );
...
void TestMerginApi::deleteLocalProject( const MerginApi *api, const QString &projectId )
{
  const LocalProject project = api->getLocalProject( projectId );
...
void TestMerginApi::deleteLocalDir( const MerginApi *api, const QString &dirPath )
{
  QDir dir( api->projectsPath() + "/" + dirPath );
  QVERIFY( dir.removeRecursively() );
}

void TestMerginApi::downloadRemoteProject( MerginApi *api, const QString &projectFullName, const QString &projectId )
{
  int serverVersion;
  downloadRemoteProject( api, projectFullName, projectId, serverVersion );
}

void TestMerginApi::downloadRemoteProject( MerginApi *api, const QString &projectFullName, const QString &projectId, int &serverVersion )
{
  QSignalSpy spy( api, &MerginApi::syncProjectFinished );
  api->pullProject( projectFullName, projectId );
  QCOMPARE( api->transactions().count(), 1 );
  QVERIFY( spy.wait( TestUtils::LONG_REPLY * 5 ) );
  serverVersion = serverVersionFromSpy( spy );
}

void TestMerginApi::uploadRemoteProject( MerginApi *api, const QString &projectFullName, const QString &projectId )
{
  int serverVersion;
  uploadRemoteProject( api, projectFullName, projectId, serverVersion );
}

void TestMerginApi::uploadRemoteProject( MerginApi *api, const QString &projectFullName, const QString &projectId, int &serverVersion )
{
  api->pushProject( projectFullName, projectId );
  QSignalSpy spy( api, &MerginApi::syncProjectFinished );
...
{
  QFile f( filename );
  const bool ok = f.open( QIODeviceBase::WriteOnly );
  Q_ASSERT( ok );
  f.write( data );
  f.flush();
  f.close();
...
QByteArray TestMerginApi::readFileContent( const QString &filename )
{
  ...
  const bool ok = f.open( QIODeviceBase::ReadOnly );
  Q_ASSERT( ok );
...
void TestMerginApi::createLocalProject( const QString &projectDir )
{
  ...
  {
    qDebug() << QString( "Failed to create directory for %1" ).arg( projectDir );
  };
  const bool r0 = QFile::copy( mTestDataPath + "/diff_project/base.gpkg", projectDir + "/base.gpkg" );
...
void TestMerginApi::testServerError() const
{
  QString msg = mApi->extractServerErrorMsg( "{\"detail\": \"Some error occurred.\"}" );
  QCOMPARE( msg, QStringLiteral( "Some error occurred." ) );

  msg = mApi->extractServerErrorMsg( "{\"name\": \"Some error occurred.\"}" );
...
void TestMerginApi::testParseVersion() const
{
  int major, minor;
...
{
  const QString projectName = "testUpdateProjectMetadataRole";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  ...
  const LocalProject projectInfo = mApi->localProjectsManager().projectFromProjectId( projectId );
  QVERIFY( projectInfo.isValid() );

  // Test 1: Initial role should be 'owner'
  QString cachedRole = mApi->getCachedProjectRole( projectId );
  QCOMPARE( cachedRole, QString( "owner" ) );

  // Test 2: Update cached role to 'reader'
  const QString newRole = "reader";
  const bool updateSuccess = mApi->updateCachedProjectRole( projectId, newRole );
  QVERIFY( updateSuccess );

  // Verify role was updated in cache
  cachedRole = mApi->getCachedProjectRole( projectId );
  ...
  mApi->reloadProjectRole( projectId );
  QVERIFY( spy.wait() );
  cachedRole = mApi->getCachedProjectRole( projectId );
  QCOMPARE( cachedRole, QString( "owner" ) );
...
void TestMerginApi::testDownloadWithNetworkError()
{
  ...
  const QString projectName = "testDownloadRetry";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  createRemoteProject( mApiExtra, mWorkspaceName, projectName, mTestDataPath + "/" + TEST_PROJECT_NAME + "/" );
  ...
  for ( QNetworkReply::NetworkError networkError : errorsToTest )
  {
    ...
    LocalProject localProject = mApi->localProjectsManager().projectFromProjectId( projectId );
...
void TestMerginApi::testDownloadWithNetworkErrorRecovery()
{
  ...
  const QString projectName = "testDownloadRetryRecovery";
  const QString projectFullName = CoreUtils::getFullProjectName( mWorkspaceName, projectName );
  createRemoteProject( mApiExtra, mWorkspaceName, projectName, mTestDataPath + "/" + TEST_PROJECT_NAME + "/" );
...
{
  // 1 => valid JSON
  QString tempFilePath = QDir::tempPath() + "/test_valid_config.json";
  {
    QFile file( tempFilePath );
    QVERIFY( file.open( QIODevice::WriteOnly ) );
    const QByteArray data = "{\"input-selective-sync\": true, \"input-selective-sync-dir\": \"photos\"}";
    file.write( data );
    file.close();
  }
  ...
  tempFilePath = QDir::tempPath() + "/test_invalid_config.json";
  {
    QFile file( tempFilePath );
    QVERIFY( file.open( QIODevice::WriteOnly ) );
    const QByteArray data = "this is not valid JSON";
    file.write( data );
    file.close();
  }
...
void TestMerginApi::testHasLocalChangesWithSelectiveSyncEnabled()
{
  ...
  bool result = MerginApi::hasLocalChanges( oldServerFiles, localFilesNoChange, projectDir, config );
  QVERIFY( !result );
  ...
  result = MerginApi::hasLocalChanges( oldServerFiles, localFilesChanged, projectDir, config );
  QVERIFY( result );
}
...
{
  // temporary project directory
  const QString projectName = "testHasLocalProjectChanges";
  QTemporaryDir tempDir;
  QVERIFY( tempDir.isValid() );
  QString projectDir = tempDir.path();
