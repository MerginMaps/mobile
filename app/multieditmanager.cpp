...
bool MultiEditManager::applyEdits()
{
  ...
  // toMap() will exclude null and multi-edited values
  ...
  for ( const FeatureLayerPair &pair : pairs )
  {
    ...
    success &= mLayer->changeAttributeValues( feature.id(), attrs ); // todo: do we need to pass a context??
    // todo: Are fields with "Apply default value on update" expressions handled?
